...
async function fetchWithTimeout(url: string): Promise<Response> {
  ...
  try {
    return await fetch(url, {
      ...
      headers: {
        "User-Agent":
          "Mozilla/5.0 (compatible; BoitenLuhrsKlantenservice/1.0)",
        Accept:
...
async function loadSitemap(
  ...
  try {
    console.log(
      `[ExternalContext] Sitemap ophalen: ${sitemapUrl}`,
    );
    ...
    if (!response.ok) {
      console.log(
        `[ExternalContext] Sitemap mislukt (${response.status}): ${sitemapUrl}`,
      );
      return [];
    }
    ...
    for (const location of locations) {
      ...
      if (looksLikeSitemap(location)) {
        const nested = await loadSitemap(
          location,
          depth + 1,
        );
    ...
    console.error(
      `[ExternalContext] Sitemap fout: ${sitemapUrl}`,
      error,
    );
 ...
 */
async function getAllExternalUrls(): Promise<string[]> {
 ...
 */
async function fetchExternalPage(
  url: string,
  terms: string[],
  ...
  try {
    console.log(
      `[ExternalContext] Pagina ophalen: ${url}`,
    );
    ...
    if (!response.ok) {
      console.log(
        `[ExternalContext] Pagina mislukt (${response.status}): ${url}`,
      );
      return null;
    }
    ...
    console.error(
      `[ExternalContext] Fout bij pagina ${url}:`,
      error,
    );
...
export async function getExternalContext(
  ...
  console.log(
    `[ExternalContext] Zoeken voor vraag: "${userMessage}"`,
  );

  const baseTerms =
    getQueryTerms(userMessage);

  const terms =
    expandQueryTerms(baseTerms);

  if (terms.length === 0) {
    console.log(
      "[ExternalContext] Geen bruikbare zoektermen.",
    );
  ...
  console.log(
    `[ExternalContext] Zoektermen: ${terms.join(", ")}`,
  );
   ...
   */
  const urls =
    await getAllExternalUrls();

  console.log(
    `[ExternalContext] ${urls.length} externe URL's beschikbaar.`,
  );
  ...
  if (results.length === 0) {
    console.log(
      "[ExternalContext] Geen relevante externe informatie gevonden.",
    );
  ...
  console.log(
    `[ExternalContext] ${results.length} relevante bron(nen) gevonden.`,
  );
