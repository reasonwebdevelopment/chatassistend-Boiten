(function(){if(window.__boitenluhrsChat)return;window.__boitenluhrsChat=!0;function e(){let e=document.querySelectorAll(`script[src]`);for(let t of Array.from(e))if(t.src.includes(`chatbot.js`)){let e=new URL(t.src);return`${e.protocol}//${e.host}`}return`http://localhost:3000`}let t=`${e()}/faq.json`,n=document.createElement(`style`);n.textContent=`
  ...
  `,document.body.appendChild(r);class i{data=null;STOP_WORDS=new Set([`de`,`het`,`een`,`van`,`is`,`wat`,`hoe`,`kan`,`ik`,`en`,`op`,`in`,`te`]);async load(){if(this.data)return this.data;try{let e=await fetch(t);if(!e.ok)throw Error(`${e.status}`);this.data=(await e.json()).faq??[]}catch{this.data=[]}return this.data}findAnswer(e){if(!this.data?.length)return;let t=e.toLowerCase(),n,r=0;for(let e of this.data){let i=e.vraag.toLowerCase().split(` `).filter(e=>e.length>2&&!this.STOP_WORDS.has(e));if(!i.length)continue;let a=i.filter(e=>t.includes(e)).length/i.length;a>r&&(r=a,n=e)}return r>=.3?n:void 0}}class a{conversationId=null;async getResponse(e){try{let t=await fetch(`https://boitenchat-355e0694e40b.herokuapp.com/api/chat`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({message:e,conversation_id:this.conversationId})});if(!t.ok){let e=`Server fout (${t.status})`;try{let n=await t.json();n?.error&&(e=n.error)}catch{}throw Error(e)}let n=await t.json();return n.conversation_id&&(this.conversationId=n.conversation_id),n.reply??`Sorry, ik kon geen antwoord vinden.`}catch(e){return e instanceof TypeError?`De chatserver is momenteel niet bereikbaar. Probeer het later opnieuw.`:`Sorry, ${e instanceof Error?e.message:String(e)}`}}}class o{faq=new i;ai=new a;delay(){return new Promise(e=>setTimeout(e,Math.floor(Math.random()*1200)+800))}async getResponse(e){await this.faq.load();let t=this.faq.findAnswer(e);return t?(await this.delay(),t.antwoord):this.ai.getResponse(e)}}let s=document.getElementById(`bl-chat-bubble`),c=document.getElementById(`bl-chat-popup`),l=document.getElementById(`bl-chat-messages`),u=document.getElementById(`bl-chat-input`),d=document.getElementById(`bl-chat-send`),f=c.querySelector(`.bl-close-btn`),p=new o;function m(){return new Date().toLocaleTimeString(`nl-NL`,{hour:`2-digit`,minute:`2-digit`})}function h(e,t){let n=document.createElement(`div`);n.className=`bl-msg bl-msg--${t}`;let r=document.createElement(`div`);r.className=`bl-msg__bubble`,r.textContent=e;let i=document.createElement(`div`);i.className=`bl-msg__time`,i.textContent=m(),n.appendChild(r),n.appendChild(i),l.appendChild(n),l.scrollTop=l.scrollHeight}function g(){let e=document.createElement(`div`);e.className=`bl-msg bl-msg--bot bl-typing`;let t=document.createElement(`div`);t.className=`bl-msg__bubble`;for(let e=0;e<3;e++){let e=document.createElement(`span`);e.className=`bl-typing-dot`,t.appendChild(e)}return e.appendChild(t),l.appendChild(e),l.scrollTop=l.scrollHeight,e}function _(){c.classList.add(`is-open`),c.setAttribute(`aria-hidden`,`false`),s.classList.add(`is-open`),u.focus()}function v(){c.classList.remove(`is-open`),c.setAttribute(`aria-hidden`,`true`),s.classList.remove(`is-open`)}async function y(){let e=u.value.trim();if(!e)return;u.value=``,u.disabled=!0,d.disabled=!0,h(e,`user`);let t=g(),n=await p.getResponse(e);t.remove(),h(n,`bot`),u.disabled=!1,d.disabled=!1,u.focus()}s.addEventListener(`click`,()=>c.classList.contains(`is-open`)?v():_()),f.addEventListener(`click`,v),d.addEventListener(`click`,y),u.addEventListener(`keydown`,e=>{e.key===`Enter`&&!e.shiftKey&&(e.preventDefault(),y())})})();